...
```
Archer:          #E8C547 (warm gold) + #8B6914 (dark gold trim)
Wizard:          #6B4FBB (royal purple) + #9B7FEB (light purple glow)
Cannibal:        #B84C4C (blood red) + #DEB887 (bone cream)
Slime:           #4CBB7F (emerald green) + #2D7A4F (darker green)
```
...
### Rarity Colors (for loot glow/borders)
```
Common:          #B0B0B0 (grey)
...
- **Idle pose:** Standing, bow drawn
- **Color:** Gold body, brown leather details, cream skin peek
- **Animations needed:**
  - Idle (subtle breathing, 4 frames)
  - Attack (bow draw + release, 4 frames)
  - Hit flash (white overlay, 2 frames)
  - Death (fall + fade)

### 🔮 Wizard
- **Shape:** Rounder, wider base (robes), pointed hat creates tall silhouette
- **Key features:** Wizard hat, flowing robes, floating orb/staff
- **Color:** Deep purple robes, light purple magic glow, gold trim
- **Animations:** Same set as Archer, attack = staff raise + magic burst

### 🦴 Cannibal
- **Shape:** Stocky, wide, hunched over — feels heavy/dangerous
- **Key features:** Bone accessories (necklace, arm bands), ragged clothing, wild hair
- **Color:** Blood red base, cream bone accents, dark leather wraps
- **Animations:** Same set, attack = bone throw (overhead hurl), consume corpse (special)

### 🟢 Slime
- **Shape:** Blob. Round bottom, slightly irregular top edge. Jiggles.
- **Key features:** Two simple eyes, slight sheen/highlight, bouncy
- **Color:** Emerald green with lighter green highlight spot
- **Animations:** Idle = constant subtle jiggle (squash/stretch, 6 frames), attack = spit animation, grows larger with power ups (scale sprite up)
...
- **Loot Orb:** Larger glowing circle, pulses, color matches rarity
- **Legendary drop:** Gold orb + expanding ring pulse + screen-edge flash
...
- **Gamba legendary:** Gold flash overlay + screen shake
...
│                                              │
│                                              │
│ [Item1] [Item2] [Item3]     Rerolls: 2  🎲  │
└──────────────────────────────────────────────┘
```

### Gamba Roll Panel
- Center screen, dark panel, rounded
- Rarity border glow (animates through colors during "roll")
- Item icon in center, large
- Item name + description below
- KEEP / REROLL buttons at bottom
- **Flash animation:** Panel border color settles on final rarity
...
1. **Archer hero** (idle + attack sprites) — replace the placeholder circle
2. **Goblin, Skeleton, Bat** sprites — replace colored shapes
3. **Arrow projectile** — replace the white rectangle
4. **XP and Loot orbs** — simple but polished
5. **Death poof effect** — spritesheet
6. **Ground texture** — basic arena floor
7. **Item icons** (10 items) — for gamba UI
8. **UI panels** — replace default Godot grey
9. **Remaining heroes** (Wizard, Cannibal, Slime)
10. **Remaining enemies** + bosses
